 ...
 * @details Gère une carte des phéronomes avec leurs mis à jour ( dont
 * l'évaporation )
 *
 */
class pheronome {
public:
  ...
  pheronome(size_t dim, const position_t &pos_food, const position_t &pos_nest,
      ...
      : m_dim(dim), m_stride(dim + 2), m_alpha(alpha), m_beta(beta),
        ...
        m_buffer_pheronome(), m_pos_nest(pos_nest), m_pos_food(pos_food) {
  ...
  void do_evaporation() {
    // this is a memory-bound operation, not a cpu-bound one, doing
    // parallelization here serves no purpose
    #pragma omp parallel
    {
        #pragma omp master
        std::cout << "Threads active: " << omp_get_num_threads() << '\n';
    }
...
#pragma omp parallel for collapse(2)
    ...
    std::cout << "wall time for execution " << (end - begin)
              << " s per pheremone update\n";
  }
