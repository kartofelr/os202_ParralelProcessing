...
class Visualizer3D:
        ...
        while sdl2.SDL_PollEvent(ctypes.byref(event)) != 0:
            ...
            # Événement mouvement de souris
                ...
                if self.mouse_dragging:
                    ...
                    self.camera_rotation_x = np.clip(
                        self.camera_rotation_x, -89.0, 89.0
                    )
    ...
    def run_parallel(self, communicator, rank, updater=None, dt=0.001):
        """
        ...
        if rank == 0:
            ...
            while self.running:
                ...
                if communicator.Iprobe(source=1, tag=11):
                    new_data = communicator.recv(source=1, tag=11)
                    self.update_points(new_data)

                self._render()

            # dire aux travailleur de s'arreter
            communicator.send(None, dest=1, tag=99)
            self.cleanup()

        elif rank == 1:
            while True:
                if communicator.Iprobe(source=0, tag=99):
                    self.cleanup()
                    break

                if updater is not None:
                    result = updater(dt)
                    communicator.send(result, dest=0, tag=11)
