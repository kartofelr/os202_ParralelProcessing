...
class Visualizer3D:
    ...
    def run_parallel(self, updater=None, dt=0.001):
        ...
        if rank == 0:
            ...
            while self.running:
                ...
                # 1. Check if the Worker has finished a calculation
                # tag=11 is used to identify "Data" messages
                if comm.Iprobe(source=1, tag=11):
                    new_data = comm.recv(source=1, tag=11)
                    self.update_points(new_data)
                
                # 2. Always render (even if no new data, keeps camera smooth)
                self._render()

            # Shutdown signal to worker
            comm.send(None, dest=1, tag=99) 
            self.cleanup()

        elif rank == 1:
            while True:
                # 1. Check if Master sent a shutdown signal (tag 99)
                if comm.Iprobe(source=0, tag=99):
                    break
                
                # 2. Calculate as fast as possible
                if updater is not None:
                    result = updater(dt)
                    # 3. Send to Master (standard send is fine here, 
                    # but Master only recvs when ready)
